      ...
      <div class="card-body p-4">
        ...
        <div class="text-center mb-4">
          @if($detail->course->img)
            <img src="{{ asset('storage/' . $detail->course->img) }}" alt="cover" class="rounded-4 shadow-sm"
        ...
        <div class="mb-4">
          <h5 class="fw-bold mb-3">
            <i class="fas fa-user-tie me-2 text-warning"></i>Mentor
          </h5>
          @if($detail->instructors && $detail->instructors->count())
            <ul class="list-unstyled mb-0">
              @foreach($detail->instructors as $instructor)
                <li class="d-flex align-items-center mb-2">
                  <img
                    src="{{ $instructor->avatar_path ? asset('storage/' . $instructor->avatar_path) : '/default-avatar.png' }}"
                    class="rounded-circle me-2 shadow-sm" style="width:40px;height:40px;object-fit:cover;">
                  <div>
                    <span class="fw-semibold">{{ $instructor->user->name }}</span>
                    @if($instructor->primary_skill)
                      <small class="text-muted d-block">{{ $instructor->primary_skill }}</small>
                    @endif
                  </div>
                </li>
              @endforeach
