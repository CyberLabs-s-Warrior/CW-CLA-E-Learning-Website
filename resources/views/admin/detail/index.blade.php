        ...
        <table class="table table-hover align-middle mb-0">
          ...
          <tbody>
            ...
            <tr>
              ...
              {{-- Mentor --}}
              <td>{{ $detail->course->instructors->pluck('name')->join(', ') ?: '-' }}</td>
              ...
              <td class="text-center">
                <div class="d-flex flex-wrap justify-content-center gap-2">
                  <a href="{{ route('admin.detail.show',$detail) }}" class="btn btn-sm btn-outline-info rounded-pill px-3 shadow-sm"><i class="fas fa-eye"></i></a>
                  <a href="{{ route('admin.detail.edit',$detail) }}" class="btn btn-sm btn-outline-warning rounded-pill px-3 shadow-sm"><i class="fas fa-edit"></i></a>
                  <button class="btn btn-sm btn-outline-danger rounded-pill px-3 shadow-sm" data-bs-toggle="modal" data-bs-target="#modalHapus{{ $detail->id }}"><i class="fas fa-trash-alt"></i></button>
                    ...
                    <div class="modal-dialog modal-dialog-centered">
                      <div class="modal-content rounded-4 shadow">
                        ...
                        <div class="modal-body">
                          <p class="mb-0">Apakah Anda yakin ingin menghapus kursus <strong>{{ $detail->course->name ?? '-' }}</strong>?</p>
                        </div>
                        <div class="modal-footer border-0">
                          <button type="button" class="btn btn-secondary rounded-pill px-3" data-bs-dismiss="modal">Batal</button>
                          <form action="{{ route('admin.detail.destroy',$detail) }}" method="POST" onsubmit="return showSpinner(this,{{ $detail->id }})">
                            @csrf @method('DELETE')
                            <button type="submit" class="btn btn-danger rounded-pill px-3 d-flex align-items-center gap-2" id="btnDelete{{ $detail->id }}">
