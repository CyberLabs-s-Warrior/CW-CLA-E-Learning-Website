    ...
    <div class="card border-0 shadow-sm rounded-4">
      <div class="card-body">
        <form action="{{ route('admin.detail.update', $detail) }}" method="POST" id="course-form">
          ...
          {{-- Preview Data Course (sama seperti create) --}}
...
@push('scripts')
  ...
  <script>
    // Init CKEditor (hanya field yang ada)
    ...
    // Preview via AJAX (sama dengan create)
    ...
    function renderPreview(data) {
      ...
      (data.lessons || []).forEach(l => {
        ...
        li.innerHTML = `<span class="fw-semibold">${l.title}</span>
                        <span class="text-muted small"> (${l.duration_formatted})</span>`;
        previewLessons.appendChild(li);
      });
      ...
      (data.instructors || []).forEach(i => {
        ...
        previewInstructors.appendChild(li);
      });
    }

    function fetchPreview(courseId){
      if(!courseId){ previewBox.classList.add('d-none'); return; }
    ...
    // On load (preselect dari data lama)
