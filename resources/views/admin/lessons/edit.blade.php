...
<div class="container-fluid py-4">
  ...
  <div class="card border-0 shadow-sm rounded-4">
    <div class="card-body">
      <form action="{{ route('admin.lessons.update', $lesson->id) }}" method="POST" enctype="multipart/form-data" class="row g-3" id="lesson-form">
        @csrf
        @method('PUT')
        ...
        <div class="col-md-6">
          ...
          <select id="module_select" class="form-select shadow-sm" required>
            @if($lesson->detail_course)
              <option value="{{ $lesson->detail_course->id }}" data-name="{{ $lesson->detail_course->module_name }}" selected>
        ...
        <input type="hidden" name="detail_courses_id" id="detail_courses_id" value="{{ $lesson->detail_courses_id }}">
        {{-- Hidden module_name --}}
        <input type="hidden" name="module_name" id="module_name" value="{{ $lesson->module_name }}">
        ...
        <div class="col-md-12">
          ...
          <textarea name="content" id="content" rows="4" class="form-control shadow-sm">{{ old('content', $lesson->content) }}</textarea>
        </div>
  ...
  courseSelect.addEventListener('change', function () {
    ...
    fetch(`/admin/course/${courseId}/modules`)
      ...
      .then(data => {
        ...
        data.forEach(item => {
          const selected = item.detail_courses_id == detailCourseInput.value ? 'selected' : '';
