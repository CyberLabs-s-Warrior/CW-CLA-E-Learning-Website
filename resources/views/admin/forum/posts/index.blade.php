...
<script>
  ...
  (function(){
    ...
    async function bulkAction(kind){
      const ids = Array.from(tbody.querySelectorAll('.tp-row:checked')).map(cb => Number(cb.value));
      if(!ids.length) return alert('Belum ada yang dipilih.');

      const url = kind === 'restore'
        ? `{{ route('admin.forum.posts.bulkRestore') }}`
        : `{{ route('admin.forum.posts.bulkForceDelete') }}`;
