{{-- resources/views/admin/showcase/create.blade.php --}}
          ...
          <div class="card-body p-3 p-md-4">
            ...
            <div class="mb-3">
              ...
              <select name="user_id" id="studentSelect" class="form-select" required>
                <option value="" disabled {{ old('user_id') ? '' : 'selected' }}>— Cari / pilih student —</option>
          ...
          <div class="card-body p-3 p-md-4">
            ...
            <div class="mb-2">
              <div class="ratio ratio-16x9 border rounded-3 bg-light d-flex align-items-center justify-content-center overflow-hidden" id="previewWrap">
                <img id="imgPreview" alt="Preview cover" class="w-100 h-100 object-fit-cover d-none">
            ...
            <div class="form-text">Maks 2MB — JPG/PNG/WebP. Gunakan rasio 16:9 agar pas.</div>
...
@push('styles')
  {{-- Tom Select (searchable select yang ringan) --}}
...
@push('scripts')
  ...
  <script>
    (function(){
      // Searchable select (local options)
      ...
      input.addEventListener('change', (e)=>{
        ...
        if(!file){ img.classList.add('d-none'); ph.classList.remove('d-none'); return; }
      ...
      // ====== OPTIONAL (AJAX search bila student sangat banyak) ======
      // Uncomment kalau kamu sediakan endpoint JSON:
      /*
      new TomSelect('#studentSelect',{
        valueField: 'id',
        labelField: 'label',
        ...
        load: function(query, callback) {
          ...
          fetch(`/admin/api/students?q=${encodeURIComponent(query)}`)
            .then(res => res.json())
            .then(json => callback(json)) // [{id:1,label:"Nama — email"}, ...]
            .catch(() => callback());
        },
        render: {
          option: (d, esc) => `<div><div class="fw-semibold">${esc(d.label)}</div></div>`
        }
      });
      */
    })();
