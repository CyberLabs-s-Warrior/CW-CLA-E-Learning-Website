{{-- resources/views/admin/showcase/edit.blade.php --}}
...
<div class="container-fluid py-3 py-md-4">
  ...
  <form action="{{ route('admin.showcases.update', $showcase) }}" method="POST" enctype="multipart/form-data" id="showcaseEditForm">
    @csrf @method('PUT')
            ...
            <div class="mb-3">
              ...
              <select name="user_id" id="studentSelect" class="form-select" required>
                ...
                @foreach($students as $s)
                  <option value="{{ $s->id }}" {{ old('user_id',$showcase->user_id)==$s->id?'selected':'' }}>
                    {{ $s->name }} — {{ $s->email }}
                  </option>
                @endforeach
              </select>
          ...
          <div class="card-body p-3 p-md-4">
            ...
            <div class="mb-2">
              <div class="ratio ratio-16x9 border rounded-3 bg-light d-flex align-items-center justify-content-center overflow-hidden" id="previewWrap">
                <img id="imgPreview" alt="Preview cover" class="w-100 h-100 object-fit-cover {{ $showcase->image_path ? '' : 'd-none' }}"
                     src="{{ $showcase->image_path ? asset('storage/'.$showcase->image_path) : '' }}">
                <div id="previewPlaceholder" class="text-muted small {{ $showcase->image_path ? 'd-none' : '' }}">
